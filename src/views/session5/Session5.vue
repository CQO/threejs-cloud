...
export default defineComponent({
  ...
  setup(props) {
    ...
    // X轴和Y轴平移的随机参数
    const RandomPositionX = 280;
    const RandomPositionY = 60;
    // 背景色，目前为天蓝色
    ...
    function init() {
      ...
      for (var i = 0; i < CloudCount; i++) {
        ...
        instanceGeometry.translate(Math.random() * RandomPositionX, (-Math.random() * RandomPositionY) + 0, i * perCloudZ);
      ...
      const normal = new THREE.TextureLoader().load( 'https://threejs.org/examples/models/3ds/portalgun/textures/normal.jpg' );

      const loader = new TDSLoader( );
      loader.setResourcePath( 'https://threejs.org/examples/models/3ds/portalgun/textures/' );
      loader.load( 'https://threejs.org/examples/models/3ds/portalgun/portalgun.3ds', function ( object ) {
        feiTingObject = object
    ...
    function animate() {
      ...
      if (feiTingObject) feiTingObject.position.z = camera.position.z - 200
      // controls.update();
